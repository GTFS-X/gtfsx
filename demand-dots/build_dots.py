...
def tippecanoe_command(inputs: str = "../tiles/ldjson/*.ldjson",
    ...
    The inputs are piped through `--cat-verified`, NOT through a bare `cat`. That
    is deliberate and load-bearing: see the concatenation gate below. A plain cat
    will happily splice a stale state into the archive and no downstream check can
    tell afterwards, so the concat step validates every sidecar's config_hash and
    refuses to emit anything if one disagrees.
    """
    out = output or f"../tiles/{TILESET_ARCHIVE}.pmtiles"
    return (
        f"./.venv/bin/python build_dots.py --cat-verified '{inputs}' \\\n"
        f"  | tippecanoe --output={out} \\\n"
...
# ─── The concatenation gate ───────────────────────────────────────────────────
#
# THIS IS THE GUARD THAT CANNOT BE DROPPED BY REWRITING THE SHELL SCRIPT.
#
# The nationwide archive is one `cat` of ~52 per-state .ldjson files. That cat is
# the exact point where a stale state becomes indistinguishable from a fresh one:
# once the bytes are concatenated, every downstream check sees a single coherent
# stream and passes. The tiles come out internally consistent and simply WRONG —
# the old schema in the states that were reused, the new one in the states that
# were rebuilt, and no error raised anywhere.
#
# build_all_states.sh has a fingerprint check on the per-state skip, and it is
# worth having (it makes re-runs cheap and it explains itself). But a bash check
# is only as durable as the next person to rewrite the script — it has already
# been lost once. So the AUTHORITY lives here, not there:
#
#   * The tippecanoe command generated by --emit-tile-cmd does not `cat` the
#     glob. It pipes through `--cat-verified`, which REFUSES to emit a single
#     byte unless every input's sidecar matches the current config_hash.
#   * So the concatenation cannot happen on a mismatched set no matter what the
#     shell script says, and no matter who retypes it out of a runbook.
#   * Delete the bash guard entirely and the worst case is a slow rebuild, not a
#     corrupt tileset: the gate turns silent corruption into a hard, loud stop.

class StaleInputError(RuntimeError):
    """The .ldjson set about to be concatenated is not coherent with this
    pipeline's config. Refuse to build rather than publish mixed-schema tiles."""


def _input_status(path: Path, want: str) -> tuple[str, str]:
    """Classify one .ldjson input. Returns (status, detail); status 'ok' passes."""
    if not path.exists() or path.stat().st_size == 0:
        return "EMPTY", "file is missing or zero-length"

    meta_path = path.with_suffix(path.suffix + ".meta.json")
    if not meta_path.exists():
        return "NO-SIDECAR", (
            "no .meta.json sidecar, so the schema and density it was built with "
            "are unknowable — it predates the sidecar and cannot be trusted")
    try:
        meta = json.loads(meta_path.read_text())
    except Exception as exc:
        return "UNREADABLE", f"sidecar will not parse ({exc})"

    # Same test verify_tiles.py applies to the built archive, hoisted to run
    # BEFORE the bytes are concatenated instead of after they are tiled.
    if "code_dots" not in meta:
        return "PRE-ATTRIBUTE-DOTS", (
            "sidecar has no `code_dots` — this is an OLD class-string build "
            "(properties.class), not attribute dots (properties.d)")

    have = meta.get("config_hash", "")
    if have != want:
        return "CONFIG-MISMATCH", (
            f"built with config {have or '(unrecorded)'}, current is {want}")
    return "ok", have


def validate_tile_inputs(paths, scale: float = 1.0, quiet: bool = False) -> list[Path]:
    """Hard-fail unless EVERY input was built by this exact pipeline config.

    Stale, sidecar-less and pre-attribute-dots files are all rejected. Raises
    StaleInputError listing every offender — the whole point is that the failure
    is loud and names names, because silence is what made this dangerous.
    """
    paths = [Path(p) for p in paths]
    if not paths:
        raise StaleInputError(
            "no .ldjson inputs matched — refusing to build an empty tileset")

    want = config_hash(scale)
    bad: list[tuple[Path, str, str]] = []
    for p in sorted(paths):
        status, detail = _input_status(p, want)
        if status != "ok":
    ...
    if bad:
        lines = [
            "",
            "╔══════════════════════════════════════════════════════════════════╗",
            "║  REFUSING TO CONCATENATE — STALE / MISMATCHED DOT INPUTS         ║",
            "╚══════════════════════════════════════════════════════════════════╝",
            "",
            f"current pipeline config_hash : {want}",
            f"inputs checked               : {len(paths)}",
            f"inputs REJECTED              : {len(bad)}",
            "",
        ]
        for p, status, detail in bad:
            lines.append(f"  [{status}] {p.name}")
            lines.append(f"      {detail}")
        lines += [
            "",
            "Concatenating these would have produced a tileset that is internally",
            "consistent and WRONG: the old schema in the reused states, the new one",
            "in the rebuilt states, and nothing downstream able to tell. Rebuild the",
            "states above (build_all_states.sh rebuilds any state whose fingerprint",
            "does not match — it will not skip them), then build the tileset.",
            "",
        ]
        raise StaleInputError("\n".join(lines))

    if not quiet:
        print(f"✓ {len(paths)} input(s) all built with config {want}",
              file=sys.stderr)
    return sorted(paths)
...
def cat_verified(pattern: str, scale: float = 1.0) -> None:
    ...
    pipeline that has already started tiling.
    """
...
def main():
    ...
    parser.add_argument("--cat-verified", metavar="GLOB",
                        help="Concatenate the .ldjson files matching GLOB to stdout "
                             "for tippecanoe — but ONLY if every one of them was "
                             "built with the current config_hash. Hard-errors on a "
                             "stale, sidecar-less or pre-attribute-dots input "
                             "instead of splicing it silently into the archive. "
                             "Quote the glob so Python expands it, not the shell.")
    parser.add_argument("--verify-inputs", metavar="GLOB",
                        help="Same check as --cat-verified, but report and exit "
                             "without emitting any data. Use as a preflight.")
    ...
    # The concat gate. Both paths exit non-zero on a mismatched set, so a build
    # pipeline that ignores the failure still cannot produce a tileset.
    if args.cat_verified or args.verify_inputs:
        pattern = args.cat_verified or args.verify_inputs
        try:
            if args.cat_verified:
                cat_verified(pattern, args.density_scale)
            else:
                paths = validate_tile_inputs(
                    globlib.glob(pattern), args.density_scale)
                print(f"✓ {len(paths)} input(s) coherent — safe to concatenate")
