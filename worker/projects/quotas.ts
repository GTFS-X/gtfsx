...
export async function countSnapshots(env: Env, projectId: string): Promise<number> {
  const row = await env.DB.prepare(
    `SELECT COUNT(*) AS n FROM feed_snapshot WHERE project_id = ?`,
  )
    .bind(projectId)
    .first<{ n: number }>();
  return row?.n ?? 0;
...
export type QuotaKind = 'projects' | 'snapshots' | 'blob' | 'published';
...
function quotaMessage(kind: QuotaKind, used: number, limit: number): string {
  switch (kind) {
    ...
    case 'snapshots':
      return `Snapshot limit reached for this project (${used}/${limit}). Delete older snapshots or upgrade your plan.`;
    case 'blob':
