...
(function () {
  ...
  // GTFS-Flex per-state/national aggregate reporting: retired 2026-07-31 (see
  // scripts/feed-health-publish.py's CANONICAL comment block) because the new data
  // source has no feed-centric per-state Flex count, only a per-agency boolean — an
  // agency-centric count was not a reasonable proxy for the old numbers (empirically
  // way off). Rather than publish a fabricated/zeroed aggregate next to real per-agency
  // Flex badges (a visible self-contradiction), HEADLINE.flexAvailable is an explicit
  // flag (set in write_fhdata_js()) that this code gates on to hide aggregate Flex
  // reporting (national band, state band, leaderboard column/filter, stat card, closing
  // plank) while keeping per-agency Flex badges — those stay sourced directly from
  // each row's own is_flex field and remain accurate regardless of this flag.
  const FLEX_AVAILABLE = !!(window.FH_DATA && window.FH_DATA.HEADLINE && window.FH_DATA.HEADLINE.flexAvailable);
  ...
  function renderStatBand() {
    ...
    // Gated on FLEX_AVAILABLE — see the constant's comment near the top of this file.
    if (FLEX_AVAILABLE) {
      const drPct = (HEADLINE.flexFeeds / HEADLINE.drAgencies * 100).toFixed(1);
      cards.push({ c: "teal", num: String(HEADLINE.flexFeeds), u: "",
    ...
    // NOTE: intentionally no inline grid-column override here — .stat-grid's column
    // count is responsive (4 / 2 / 1 across breakpoints in fh-styles.css); an inline
    // style would win over those @media rules and break the mobile layout. 3 cards in
    // the default 4-column desktop grid leaves one empty cell, which is a purely
    // cosmetic/temporary side effect of hiding the Flex card, not a broken layout.
  ...
  // Flex column/filter dropped while FLEX_AVAILABLE is false — a column of dashes next
  // to real per-agency Flex badges elsewhere on the page would be its own mixed signal.
  const COLS = [
    ...
    ...(FLEX_AVAILABLE ? [{ key: "flex", label: "Flex", num: true }] : []),
  ];
  ...
  const FILTERS = [
    ...
    ...(FLEX_AVAILABLE ? [{ key: "flex", label: "Publishes Flex" }] : []),
  ];
  ...
  function renderLeaderboard() {
    ...
    function showDrillDown(s) {
      ...
      if (FLEX_AVAILABLE) {
        statItems.push({ label: "GTFS-Flex Feeds", value: s.flex > 0 ? String(s.flex) : "0" });
      }
    ...
    function renderBody() {
      ...
      rows.forEach((s, i) => {
        ...
        tr.innerHTML = `
          ...
          ${FLEX_AVAILABLE ? `<td class="lb-num">${s.flex || "–"}</td>` : ""}
  ...
  function renderFlexSection() {
    ...
    if (!FLEX_AVAILABLE) {
      // Keep the section + #flex id so the nav link (top bar / footer) still lands
      // somewhere sensible, but don't assert any aggregate number — see FLEX_AVAILABLE
      ...
      return h("section", { className: "flex-band", id: "flex" },
        h("div", { className: "wrap section" },
          h("div", { className: "section-head" },
            ...
            h("h2", null, "GTFS-Flex totals are being rebuilt"),
            h("p", null,
              "Per-state and national GTFS-Flex totals are temporarily unavailable while we migrate this dashboard's data source. Individual agency Flex badges, in the leaderboard drill-downs above, remain accurate. ",
            ...
            h("dl", null,
              ...
              h("div", null,
                ...
                h("dd", null,
                  ...
                  h("em", null, "expired"),
                  " is decided from our own direct read of the agency's calendar.txt / calendar_dates.txt (falling back to feed_info.txt only when a feed carries no calendar data) whenever we can fetch that feed ourselves — this is current as of this snapshot, not a stale capture. Only when our own fetch fails outright (a timeout, a 404, a corrupt zip, and similar) do we fall back to Mobility Database's last-captured service end date instead; MDB recrawls smaller, infrequently-updated agencies on an irregular cadence — in some cases a year or more between captures — so an expired status resting specifically on that fallback can lag behind an agency that has since quietly republished a current feed."
                )
              ),
              h("div", null,
  ...
  function renderClosingCta() {
    ...
    // "Champion GTFS-Flex" plank asserts specific Flex counts (Colorado's, the national
    // total) — dropped while FLEX_AVAILABLE is false rather than published with false
    // numbers. See FLEX_AVAILABLE comment near the top of this file.
    if (FLEX_AVAILABLE) {
    ...
    // Single plank: force one column. .closing-grid is 1fr 1fr on desktop and already
    // collapses to 1fr under 980px in fh-styles.css, so this inline override agrees
    // with the responsive rule at every breakpoint rather than fighting it.
    const gridStyle = planks.length === 1 ? { gridTemplateColumns: "1fr" } : undefined;
    ...
    return h("section", { className: "closing-band", id: "closing" },
      h("div", { className: "wrap" },
        ...
        h("div", { className: "closing-grid", style: gridStyle }, ...planks)
      )
    );
  }
  ...
  function renderStateFlexSection(stateMeta) {
    ...
    if (!FLEX_AVAILABLE) {
      // Do NOT fall through to the "no agencies report GTFS-Flex" copy below — that's
      // a specific false claim for any state with real per-agency Flex badges showing
      // in the agency table above. See FLEX_AVAILABLE comment near the top of this file.
      return h("section", { className: "flex-band state-flex", id: "flex" },
        h("div", { className: "wrap section" },
          h("div", { className: "section-head" },
            h("span", { className: "eyebrow" }, "GTFS-Flex"),
            h("h2", null, "GTFS-Flex totals are being rebuilt"),
            h("p", { className: "state-flex-body" },
              "Per-state GTFS-Flex totals are temporarily unavailable while we migrate this dashboard's data source. Individual " + name + " agencies' Flex badges, in the agency table above, remain accurate. ",
              learnLink
            )
          )
        )
