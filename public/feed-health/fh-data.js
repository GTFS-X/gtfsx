// GENERATED by scripts/feed-health-publish.py — do not hand-edit.
// Source: GTFS-X/gtfs-feed-health canonical export (exports/feed_health.json).
// Source ref: main -> 52649959232b2a3c5ed059d98bedf14157f135d2   Export schemaVersion: 1.0.0   Export generatedAt: 2026-07-31T16:03:56Z
// Run date: 2026-07-31.  Re-run scripts/feed-health-publish.py to refresh (fetches
// upstream's current main fresh each run by default; --ref <sha> pins, --export-json
// <path> uses a local file offline).
(function () {
  // ---- Headline findings — recomputed from the fetched export's per-agency rows ----
  const HEADLINE = {
    noFeedPct: 45,         // % of US federally funded agencies w/ no findable GTFS feed
    agencies: 2238,          // NTD agency roster (full universe incl. territories)
    expiredPct: 10,           // % of MDB-matched feeds describing service that already ended
    validatorFailPct: 12.9, // % of MDB-matched feeds failing the canonical validator
    // GTFS-Flex aggregate reporting retired 2026-07-31 — see CANONICAL comment block in
    // scripts/feed-health-publish.py for why (feed-centric vs. agency-centric data, not
    // a reasonable proxy). flexAvailable is the explicit gate fh.js reads to HIDE the
    // aggregate Flex band/leaderboard/stat-card/closing-plank entirely (rather than
    // show a "0" a visitor could see visibly contradicted by real per-agency Flex
    // badges on the same page) — flip back to true once a real per-state breakdown
    // exists upstream. flexFeeds/flexStates are kept at 0 only for any code that reads
    // them before checking the flag; fh.js is expected to check flexAvailable first.
    flexAvailable: false,
    flexFeeds: 0,
    flexStates: 0,
    // Agencies reporting Demand Response (mode DR; DT absorbed since report_year 2019),
    // computed live from the fetched export's per-agency demand_response booleans
    // (previously a hardcoded constant from a separate NTD Service-by-Mode extract).
    drAgencies: 1924,
    refresh: "Monthly",
    asOf: "2026-07-31",
    draftDate: "July 31, 2026",
    owner: "Mark Egge",
  };

  // Size-gradient cut — % of agencies in each NTD reporting class w/ no feed anywhere
  const GRADIENT = [
    { key: "full",  label: "Full Reporters", sub: "Urbanized, full NTD reporting",  noFeedPct: 10, agencies: 524 },
    { key: "reduced",  label: "Reduced Reporters", sub: "Smaller urbanized + tribal",  noFeedPct: 37, agencies: 603 },
    { key: "rural",  label: "Rural (5311) Agencies", sub: "Non-urbanized rural service",  noFeedPct: 65, agencies: 1111 },
  ];

  // ---- Per-state rows (REAL values computed from the fetched export) ----
  // cov  = % of the state's NTD agencies with a findable GTFS feed (FTA weblink OR Mobility Database)
  // exp  = % of MDB-matched feeds in that state whose service period has already ended
  // val  = % of MDB-matched feeds in that state with at least one ERROR-severity validator notice
  // flex = RETIRED, always 0 — see HEADLINE.flexFeeds comment above
  const S = (fips, abbr, name, region, agencies, cov, exp, val, flex) =>
    ({ fips, abbr, name, region, agencies, cov, noFeed: 100 - cov, exp, val, flex });

  const STATES = [
    S("01","AL","Alabama","South",38,24,17,0,0),
    S("02","AK","Alaska","West",26,58,0,0,0),
    S("04","AZ","Arizona","West",45,67,41,6,0),
    S("05","AR","Arkansas","South",15,53,25,0,0),
    S("06","CA","California","West",209,80,9,20,0),
    S("08","CO","Colorado","West",48,85,3,13,0),
    S("09","CT","Connecticut","Northeast",20,85,0,0,0),
    S("10","DE","Delaware","Northeast",2,100,0,100,0),
    S("11","DC","District of Columbia","Northeast",3,67,50,50,0),
    S("12","FL","Florida","South",72,75,27,17,0),
    S("13","GA","Georgia","South",83,18,0,8,0),
    S("15","HI","Hawaii","West",4,100,0,33,0),
    S("16","ID","Idaho","West",14,79,0,17,0),
    S("17","IL","Illinois","Midwest",98,29,17,11,0),
    S("18","IN","Indiana","Midwest",67,39,19,6,0),
    S("19","IA","Iowa","Midwest",39,51,11,0,0),
    S("20","KS","Kansas","Midwest",85,18,12,12,0),
    S("21","KY","Kentucky","South",33,52,0,0,0),
    S("22","LA","Louisiana","South",50,24,0,0,0),
    S("23","ME","Maine","Northeast",22,64,10,20,0),
    S("24","MD","Maryland","Northeast",22,95,6,38,0),
    S("25","MA","Massachusetts","Northeast",24,92,15,5,0),
    S("26","MI","Michigan","Midwest",86,40,11,5,0),
    S("27","MN","Minnesota","Midwest",46,63,22,11,0),
    S("28","MS","Mississippi","South",25,20,0,0,0),
    S("29","MO","Missouri","Midwest",34,41,14,0,0),
    S("30","MT","Montana","West",46,35,29,29,0),
    S("31","NE","Nebraska","Midwest",59,10,0,25,0),
    S("32","NV","Nevada","West",21,52,0,33,0),
    S("33","NH","New Hampshire","Northeast",11,82,0,20,0),
    S("34","NJ","New Jersey","Northeast",28,75,10,10,0),
    S("35","NM","New Mexico","West",26,58,8,15,0),
    S("36","NY","New York","Northeast",77,91,9,12,0),
    S("37","NC","North Carolina","South",103,44,11,6,0),
    S("38","ND","North Dakota","Midwest",28,14,0,33,0),
    S("39","OH","Ohio","Midwest",66,50,16,16,0),
    S("40","OK","Oklahoma","South",36,28,0,0,0),
    S("41","OR","Oregon","West",42,98,3,6,0),
    S("42","PA","Pennsylvania","Northeast",50,70,0,0,0),
    S("44","RI","Rhode Island","Northeast",2,100,0,0,0),
    S("45","SC","South Carolina","South",26,58,12,25,0),
    S("46","SD","South Dakota","Midwest",22,18,0,0,0),
    S("47","TN","Tennessee","South",26,69,7,14,0),
    S("48","TX","Texas","South",71,76,13,27,0),
    S("49","UT","Utah","West",8,100,0,25,0),
    S("50","VT","Vermont","Northeast",9,78,0,0,0),
    S("51","VA","Virginia","South",39,95,0,7,0),
    S("53","WA","Washington","West",57,86,5,9,0),
    S("54","WV","West Virginia","South",19,95,29,14,0),
    S("55","WI","Wisconsin","Midwest",79,33,7,14,0),
    S("56","WY","Wyoming","West",27,22,25,0,0),
  ];

  // Flex leaderboard — states publishing GTFS-Flex, ranked. Naturally empty while
  // every state's flex=0 (see above); the render code already degrades cleanly when
  // this is [].
  const FLEX = STATES.filter((s) => s.flex > 0).sort((a, b) => b.flex - a.flex);

  // CTA variants keyed to feed condition — RESERVED for per-agency drill-down phase.
  // Not rendered on the public dashboard; kept here for future use.
  const CTAS = [
    { key: "edit",  verb: "Edit this feed",  label: "Edit this feed in GTFS·X",
      cond: "Clean feed", desc: "Feed validates and describes current service. Open it to refine stops, trips, and timetables.",
      tone: "teal" },
    { key: "fix",   verb: "Fix this feed",   label: "Fix this feed in GTFS·X",
      cond: "Broken or expired", desc: "Feed fails the canonical validator or describes service that has already ended. Open it to repair and re-export.",
      tone: "gold" },
    { key: "build", verb: "Build a feed",    label: "Build a feed for this agency in GTFS·X",
      cond: "No feed found", desc: "No GTFS feed any trip planner can find. Draw routes, place stops, and publish a validated gtfs.zip.",
      tone: "coral" },
  ];

  window.FH_DATA = { HEADLINE, GRADIENT, STATES, FLEX, CTAS };
})();
