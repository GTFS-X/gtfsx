...
```
feeds.gtfsbuilder.net/<slug>/                          # mini-site landing (indexable)
feeds.gtfsbuilder.net/<slug>/embed/system-map          # system overview embed
feeds.gtfsbuilder.net/<slug>/embed/route/<route_id>    # per-route map + schedule
feeds.gtfsbuilder.net/<slug>/embed/stop/<stop_id>      # per-stop "departures today"
feeds.gtfsbuilder.net/_/orgs/<org_id>/logo             # public org-logo bytes (CORS open)
```
